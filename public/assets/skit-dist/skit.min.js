...
window.Dialog={open:e=>{var d,l;(e.html=e.html.split("{{dialogs}}").join(""),document.getElementById("dialog"))?((d=document.getElementById("dialog")).classList.remove("hidden"),d.innerHTML="",(l=document.createElement("div")).classList.add("dialog-area"),l.innerHTML=e.html,e.close&&l.appendChild(c),d.appendChild(l)):((d=document.createElement("div")).setAttribute("id","dialog"),(l=document.createElement("div")).classList.add("dialog-area"),l.innerHTML=e.html,d.appendChild(l),document.body.appendChild(d));e.invisible&&l.classList.add("dialog-invisible"),Boss.evts.add(Boss.evtTouchUp(),document.getElementById("dialog-close"),(function(d){Boss.dialog.close(),e.callBack&&"function"==typeof e.callBack&&e.callBack()}))},close:function(){if(document.getElementById("dialog")){var e=document.getElementById("dialog");e.classList.add("hidden"),e.innerHTML=""}}};
...
class Modal{constructor(t){this.initX=0,this.initY=0,this.canMove=!1,this.comp=document.getElementById(t),this.move=this.comp.querySelector(".Move"),this.initMove(),this.comp.addEventListener("click",(()=>{this.setFocus()}))}initMove(){document.addEventListener("mousedown",(t=>{if(t.target==this.move){this._setZIndex(this._getZIndex()),this.canMove=!0;var e=this.move.getBoundingClientRect();this.initX=this.comp.offsetLeft+t.x-e.x,this.initY=this.comp.offsetTop+t.y-e.y,document.addEventListener("mousemove",this.handleMove)}})),document.addEventListener("mouseup",(t=>{this.canMove=!1,document.removeEventListener("mousemove",this.handleMove)}))}handleMove=t=>{!0===this.canMove&&window.requestAnimationFrame((()=>{this.comp.style.transform="translate("+(t.x-this.initX).toFixed(0)+"px, "+(t.y-this.initY).toFixed(0)+"px)"}))};_getZIndex(){var t=document.querySelectorAll("dialog"),e=0;return t.forEach((t=>{if(this.comp!=t){var i=Number(window.document.defaultView.getComputedStyle(t).getPropertyValue("z-index"));i>=e&&(e=i)}})),++e}_setZIndex(t){this.comp.style.zIndex=t}setTitle(t){this.title=t.substring(0,50),this.comp.querySelector(".Move").textContent=this.title}setContent(t){this.comp.querySelector(".ModalContent").innerHTML=t}setFocus(){this._setZIndex(this._getZIndex())}open(){this.comp.show(),this._setZIndex(this._getZIndex())}close(){this.comp.close(),this.initX=0,this.initY=0,this.comp.style.transform=null,this.comp.style.zIndex=null}}
